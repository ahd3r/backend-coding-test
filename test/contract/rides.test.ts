...
import { RidesEntity } from '../../src/models/rides';

import { getRequest, postRequest } from './base-test';

describe('testing rides endpoints', () => {
  it('checks rides in db', async () => {
    const res: request.Response = await getRequest('/rides');
  ...
  it('checks mistake in router', async () => {
    const res: request.Response = await getRequest('/ridess');
  ...
  it('correct create ride', async () => {
    ...
    const res: request.Response = await postRequest('/rides', correctDataToCreateRide);
  ...
  it('incorrect create ride', async () => {
    const res1: request.Response = await postRequest('/rides', {});

    expect(res1.status).toBe(400);
    expect(res1.body).toHaveProperty('errors');
    expect(res1.body.errors.length).toBe(10);

    const res2: request.Response = await postRequest('/rides', { start_lat: 1 });
  ...
  it('get ride by id', async () => {
    const {
      body: { data: allRides }
    }: request.Response = await getRequest('/rides');

    const res: request.Response = await getRequest(`/rides/${allRides[0].id}`);
  ...
  it('get ride by wrong id', async () => {
    const res: request.Response = await getRequest('/rides/9999');
  ...
  it('check pagination', async () => {
    ...
    for (let i = 0; i < 15; i++) {
      await postRequest('/rides', correctDataToCreateRide);
    }

    const res1: request.Response = await getRequest('/rides');
    ...
    const res2: request.Response = await getRequest('/rides?page=2&limit=16');
    ...
    const res3: request.Response = await getRequest('/rides?page=2');
